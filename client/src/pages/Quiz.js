...
function Quiz() {
  ...
  useEffect(() => {
    ...
    setCurrentScore(0);
    setHighScore(0);
    let tempCurr = currentScore;
    let temphigh = highScore;
    ...
    const drawLandmarks = (landmarksArray, recognizerResult) => {
      ...
      landmarksArray.forEach(landmarks => {
        ...
        if(firstCategoryName === "None"){
          ...
          setCurrentGesture(firstCategoryName);
          tempCurr = tempCurr+1;
          setCurrentScore(tempCurr);
          if(tempCurr > temphigh){
            setHighScore(tempCurr);
          }
          setTargetGesture(categoryNames[Math.floor(Math.random() * (categoryNames.length - 1) + 1)]);
  ...
  return (
    <div className="page p-6 text-center">
      ...
      <p className="mb-6">Test your ASL knowledge!</p>
